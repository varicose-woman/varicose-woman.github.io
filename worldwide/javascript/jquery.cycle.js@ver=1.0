...
jQuery(window).load(function(){
	...
	// recent post slider
	var recent_post_slider = jQuery('.gdl-recent-post-slider-widget');
	if( recent_post_slider.length > 0 ){
		recent_post_slider.each(function(){
			var recent_post_slider_nav = jQuery(this).siblings('.recent-post-slider-nav');
			gdl_cycle_resize(jQuery(this));
			jQuery(this).cycle({ fx: 'fade', slideResize: 1, fit: true, width: '100%',
			prev: recent_post_slider_nav.children('.prev'), next: recent_post_slider_nav.children('.next') });
		});
	}		
	...
	jQuery(window).resize(function(){ 
		...
		if( recent_post_slider ){ gdl_cycle_resize(recent_post_slider); }
	});		
